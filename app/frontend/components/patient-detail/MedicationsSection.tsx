 ...
 * Displays and manages patient medications
 */
'use client';

import { useState, useEffect } from 'react';
import { medicationService, Medication, MedicationCreate } from '@/lib/medicationService';
...
export function MedicationsSection({ patientId }: MedicationsSectionProps) {
  ...
  const fetchMedications = async () => {
    ...
    try {
      ...
      const data = await medicationService.getPatientMedications(patientId, statusFilter);
      setMedications(data);
    } catch (err: any) {
  ...
  const handleDiscontinue = async (medicationId: string) => {
    try {
      ...
      setError(err.response?.data?.detail || 'Failed to discontinue medication');
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Why this medication was prescribed"
              />
            </div>
            <div>
                ...
                <div className="flex items-center gap-2 ml-4">
                  {medication.status === 'active' && (
                    <button
                      ...
                      className="px-3 py-1 text-sm text-red-700 bg-red-50 rounded hover:bg-red-100"
                    >
                      Discontinue
                    </button>
                  )}
