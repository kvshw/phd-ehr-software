...
import { WeightBalanceCard } from '@/components/dashboard/WeightBalanceCard';
import { HeartRateCard } from '@/components/dashboard/HeartRateCard';
import { HydrationCard } from '@/components/dashboard/HydrationCard';
import { BloodCellsCard } from '@/components/dashboard/BloodCellsCard';
import { SleepPeriodicCard } from '@/components/dashboard/SleepPeriodicCard';
import { BloodTrackingCard } from '@/components/dashboard/BloodTrackingCard';
      ...
      <div className="max-w-[1600px] mx-auto px-6 py-8">
        <div className="flex gap-6">
          ...
          <div className="flex-1 space-y-6">
            {/* Specialty Banner - Shows if user has selected a specialty */}
            {user.specialty && (
              <SpecialtyBanner specialty={user.specialty} userName={user.first_name || user.email?.split('@')[0]} />
            )}

            {/* Prompt to set specialty if not set */}
            {!user.specialty && (
            ...
            {/* Specialty-based Quick Actions */}
            {user.specialty && (
              <SpecialtyQuickActions specialty={user.specialty} />
            )}

            {/* Health Metrics Grid - Show aggregate stats or remove if not meaningful */}
            {/* For now, showing patient count and key stats instead of individual health metrics */}
